...
**Scope**: both | **Doc**: [upstream-check-2026-03-12.md](upstream-check-2026-03-12.md)

Heavy merge day on Gas Town (~30 commits, 15+ merged PRs) and Dolt hardening on Beads (~30 commits). No new releases — still waiting for bd v0.59.1+. Key GT merges: event-driven polecat lifecycle with `FIX_NEEDED` state (#2633), convoy `--base-branch` (#2398), refinery merge strategy (#2609), daemon pressure checks (#2370), mail auto-nudge (#2674). GT PR #2672 confirms `--flat` flag removal — validates our same-day fix in mg. Beads adds `--design-file` flag, global PRIME.md fallback, and `bd bootstrap` auto-recovery.

**Breaking**: `--flat` flag removed (already fixed in mg). **Action items**: 1) Track bd v0.59.1+. 2) Render `FIX_NEEDED` polecat state. 3) Carry forward epic progress + bd context (blocked on v0.59.1+).

---
## 2026-03-11

**Scope**: both | **Doc**: N/A (incremental, no new research doc)

Quiet day — all bug fixes, no new releases. Beads: 11 new commits, mostly Dolt stability (test suite hardening, worktree hook paths, config precedence, metadata merge conflict auto-resolution) plus glamour v2 and huh v2 dependency migrations. Gas Town: ~25 new commits, heavy stability work (tmux socket split-brain fix, idle polecat session reaping, standing-order bead protection from AutoClose, crew specialization design doc). Notable GT fix: `gt prime --hook blocks forever on non-Claude runtimes`. New PRs of interest: bd #2509 (coexisting dep types), #2510 (milestones), #2508 (session summary).

**Breaking**: None. **Action items**: None new — carry forward from 2026-03-10 (bd v0.59.1 tracking, epic progress, bd context).
...
**Scope**: both | **Doc**: [upstream-check-2026-03-10.md](upstream-check-2026-03-10.md)

Massive activity on both repos (~30 commits each). **Critical `--json` fix (PR #2432) merged on 2026-03-08** — bd upgrade path is unblocked pending v0.59.1+ release. Beads main adds JSON contract tests, `bd context` command, `bd bootstrap` command, GitHub Issues tracker plugin, epic close guards with progress display, shared Dolt server mode, and ephemeral port allocation. Gas Town main adds `/review` command with A-F grading, formula extends/compose, background nudge queue poller, convoy cross-rig tracking fixes, and RoleDog in session identity. No new releases on either repo.

**Breaking**: bd JSON error output now structured (stderr) — review source.go error handling. **Action items**: 1) Track bd v0.59.1+ release. 2) Review JSON error handling in source.go. 3) Upgrade gt go.mod to v0.11.0. 4) Render dogs in agent roster. 5) Surface epic progress in detail panel.
